...
const getUnauthorized = (request, response) => {
  ...
  if (!id) { return getData(request, response, message, status); }
  return getData(request, response, message, status, id);
};

const getForbidden = (request, response) => getData(request, response, 'You do not have access to this content', 403, 'forbidden');

const getInternal = (request, response) => getData(request, response, 'Internal Server Error. Something went wrong.', 500, 'internalError');

const getNotImplemented = (request, response) => getData(request, response, 'A get request for this page has not been implemented yet. Check again later for updated content', 501, 'notImplemented');

const getNotFound = (request, response) => getData(request, response, 'The page you are looking for was not found', 404, 'notFound');
